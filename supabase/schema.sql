...
alter table public.point_ledger add constraint point_ledger_order_fk foreign key (order_id) references public.orders(id) on delete restrict;
...
select v.name, v.diamonds, v.price_points from (values
  ('100 جوهرة',100,300),('300 جوهرة',300,900),('500 جوهرة',500,1500),('1000 جوهرة',1000,3000),('5000 جوهرة',5000,15000)
) as v(name,diamonds,price_points)
where not exists (select 1 from public.products p where p.diamonds=v.diamonds and p.price_points=v.price_points);
...
create or replace function public.spend_points_for_order(p_store_user_id uuid,p_product_id uuid,p_player_name text,p_country text,p_game_id text)
returns public.orders language plpgsql security definer set search_path=public as $$
declare v_user public.store_users%rowtype; v_product public.products%rowtype; v_order public.orders%rowtype;
begin
  ...
  insert into public.orders(store_user_id,product_id,diamonds,price_points,player_name,country,game_id,points_before,points_after)
  values(v_user.id,v_product.id,v_product.diamonds,v_product.price_points,trim(p_player_name),trim(p_country),trim(p_game_id),v_user.points,v_user.points-v_product.price_points)
  returning * into v_order;
  update public.store_users set points=points-v_product.price_points,updated_at=now() where id=v_user.id;
  insert into public.point_ledger(store_user_id,direction,amount,order_id,reason) values(v_user.id,'debit',v_product.price_points,v_order.id,'store redemption');
  return v_order;
end;
$$;

create or replace function public.grant_watch_points(p_kick_user_id text,p_channel_slug text,p_event_key text,p_points bigint)
returns bigint language plpgsql security definer set search_path=public as $$
declare v_user public.store_users%rowtype; v_channel public.watch_channels%rowtype; v_inserted integer;
begin
  ...
  if not found then raise exception 'USER_NOT_FOUND'; end if;
  insert into public.watch_events(event_key,store_user_id,channel_slug,points) values(trim(p_event_key),v_user.id,v_channel.channel_slug,p_points) on conflict(event_key) do nothing;
  ...
  insert into public.point_ledger(store_user_id,direction,amount,source,reason) values(v_user.id,'credit',p_points,'watch','verified stream watch');
