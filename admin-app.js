(() => {
  ...
  async function load() {
    ...
    const watchEvents = data.watchEvents || [];
    ...
    document.getElementById('ledgerTable').innerHTML = `
      <div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>Kick ID</th><th>الحركة</th><th>المصدر</th><th>النقاط</th><th>السبب</th><th>الطلب</th></tr></thead><tbody>
      ${ledger.map((x) => `<tr><td>${new Date(x.created_at).toLocaleString('ar-DZ')}</td><td>${esc(x.store_user_id)}</td><td>${directionText(x.direction)}</td><td>${esc(x.source || '—')}</td><td>${money(x.amount)}</td><td>${esc(x.reason)}</td><td>${esc(x.order_id || '—')}</td></tr>`).join('') || '<tr><td colspan="7">لا توجد حركات بعد.</td></tr>'}
      </tbody></table></div>`;

    document.getElementById('watchTable').innerHTML = `
      <div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>Kick ID</th><th>القناة</th><th>دقائق المشاهدة</th><th>النقاط</th><th>Event Key</th></tr></thead><tbody>
      ${watchEvents.map((x) => `<tr><td>${new Date(x.created_at).toLocaleString('ar-DZ')}</td><td>${esc(x.kick_user_id)}</td><td>${esc(x.channel_user_id)}</td><td>${money(x.watch_minutes)}</td><td>${money(x.points_awarded)}</td><td>${esc(x.event_key)}</td></tr>`).join('') || '<tr><td colspan="6">لا توجد أحداث مشاهدة بعد.</td></tr>'}
      </tbody></table></div>`;
