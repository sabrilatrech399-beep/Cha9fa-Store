...
-- IMPORTANT: the live products.id column is bigint. Keep it bigint instead of
-- attempting to replace the primary key and breaking existing redemptions.
...
set diamonds = case
  when price_points = 300 then 100
  when price_points = 900 then 300
  when price_points = 1500 then 500
  when price_points = 3000 then 1000
  when price_points = 15000 then 5000
  else nullif(regexp_replace(coalesce(name,''),'[^0-9]','','g'),'')::integer
end
where diamonds is null;

update public.products
set image_url = '/images/diamond.png'
where image_url is null or btrim(image_url) = '';

alter table public.products alter column diamonds set not null;
alter table public.products add constraint products_diamonds_positive check (diamonds > 0) not valid;
alter table public.products validate constraint products_diamonds_positive;
...
begin
  if exists (
    ...
    if exists (select 1 from public.orders) then
      raise exception 'ORDERS_PRODUCT_ID_UUID_HAS_DATA';
    end if;
    alter table public.orders drop constraint if exists orders_product_id_fkey;
    alter table public.orders alter column product_id type bigint using product_id::text::bigint;
...
create index if not exists store_users_kick_user_id_idx on public.store_users(kick_user_id);
...
create index if not exists point_ledger_user_created_idx on public.point_ledger(store_user_id, created_at desc);
create index if not exists point_ledger_source_idx on public.point_ledger(source, created_at desc);
create index if not exists orders_user_created_idx on public.orders(store_user_id, created_at desc);
...
) v(name,diamonds,price_points)
where not exists (
  select 1 from public.products p
  where p.diamonds=v.diamonds and p.price_points=v.price_points
);
...
revoke all on function public.spend_points_for_order(uuid,bigint,text,text,text) from public,anon,authenticated;
revoke all on function public.grant_watch_points(text,text,text,bigint) from public,anon,authenticated;
grant execute on function public.spend_points_for_order(uuid,bigint,text,text,text) to service_role;
