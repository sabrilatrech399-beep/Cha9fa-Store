...
const required = ['SUPABASE_URL','SUPABASE_SERVICE_ROLE_KEY','KICK_CLIENT_ID','KICK_CLIENT_SECRET','KICK_REDIRECT_URI','SESSION_SECRET'];
...
app.post('/internal/watch-award', rateLimit('watch', RATE_LIMITS.watch), async (req,res) => {
  if (!constantTimeSecretMatch(String(req.get('x-watch-ingest-secret') || ''), WATCH_INGEST_SECRET)) {
    return res.status(401).json({ error:'WATCH_AUTH_REQUIRED' });
  }
  if (!WATCH_INGEST_SECRET || !WATCH_CHANNEL_USER_ID || !Number.isInteger(WATCH_POINTS_PER_MINUTE) || WATCH_POINTS_PER_MINUTE < 1) {
    return res.status(503).json({ error:'WATCH_SERVICE_NOT_CONFIGURED' });
  }
  ...
  const { data,error } = await supabase.rpc('award_watch_points', {
    p_event_key:eventKey.trim(), p_kick_user_id:kickUserId.trim(), p_channel_user_id:String(channelUserId), p_watch_minutes:minutes, p_points:points,
  });
  if (error) {
    const message=String(error.message||'');
