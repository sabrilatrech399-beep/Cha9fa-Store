(() => {
  ...
  async function load() {
    ...
    document.getElementById('watchTable').innerHTML = `<div class="table-wrap"><table><thead><tr><th>التاريخ</th><th>Kick ID</th><th>القناة</th><th>دقائق المشاهدة</th><th>النقاط</th><th>Event Key</th></tr></thead><tbody>${watchEvents.map((x) => `<tr><td>${new Date(x.created_at).toLocaleString('ar-DZ')}</td><td>${esc(x.kick_user_id)}</td><td>${esc(x.channel_user_id)}</td><td>${money(x.watch_minutes)}</td><td>${money(x.points_awarded)}</td><td>${esc(x.event_key)}</td></tr>`).join('') || '<tr><td colspan="6">لا توجد أحداث مشاهدة بعد.</td></tr>'}</tbody></table></div>`;
